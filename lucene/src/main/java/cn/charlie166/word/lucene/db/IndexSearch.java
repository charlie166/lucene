	...
	* @param str 查询字符串
	...
	public void queryString(String str) throws IOException, ParseException{
	    ...
	    if(contentQuery instanceof BooleanQuery){
	    	...
	    	logger.debug("[1]content:Searching for: " + thisQuery.toString(str));
	    	bqb.add(thisQuery, BooleanClause.Occur.MUST);
	    } else {
	    	logger.debug("[2]content:Searching for: " + contentQuery.toString(str));
	...
	public void doPagingSearch(IndexSearcher searcher, Query query, int hitsPerPage) throws IOException{
	    ...
	    for (int i = start; i < end; i++) {
	        ...
	        if (idStr != null) {
	        	logger.debug((i + 1) + ". " + idStr + " [score=" + hits[i].score + "]");
	        	String title = doc.get(CommonAttributes.KEY_TITLE);
	        	if (title != null) {
	        		logger.debug(CommonAttributes.KEY_TITLE + ": " + title);
	        	} else {
	        		logger.debug("no" + CommonAttributes.KEY_TITLE);
	        	}
	        	...
	        	logger.debug((i + 1) + ". " + "No " + CommonAttributes.KEY_ID + " for this document");
	        }
	    }
	}
}